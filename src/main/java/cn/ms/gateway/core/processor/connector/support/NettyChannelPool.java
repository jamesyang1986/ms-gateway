...
public class NettyChannelPool {
		...
		clientBootstrap.group(group).channel(NioSocketChannel.class)
				...
				.handler(new ChannelInitializer() {
					...
					protected void initChannel(Channel ch) throws Exception {
						...
						ch.pipeline().addLast(new IdleStateHandler(0, 0, NettyChannelPool.this.maxIdleTimeInMilliSecondes, TimeUnit.MILLISECONDS));
						ch.pipeline().addLast(new NettyChannelPoolHandler(NettyChannelPool.this));
					}
	 ...
	 * @throws Exception
	 */
	public NettyHttpResponseFuture sendRequest(InetSocketAddress route, final HttpRequest request) throws InterruptedException, IOException {
